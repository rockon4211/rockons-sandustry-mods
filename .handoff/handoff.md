# Sandustry mods — handoff (2026-09-23, updated after the laptop install)

Everything a new chat needs to pick this work up. Written at the end of the session that
built the Screensaver tracer system. Game: Sandustry v0.5.6 (Steam). Player: Brandon.
...
- Git repo (history of every mod): `/home/claude/material-studio` in the session container,
  branch `master`, plus branch `snapshot/loamcrest-2026-09-20` and tag `loamcrest-2026-09-20`
  holding the frozen map + mods. No remote. If the container is recycled, the repo is
  restored from a bundle kept on the PC (this has happened once already).
- Live edit copies: `/home/claude/sandbox-loop-mod`, `/home/claude/screensaver-mod`.
- Test harnesses: `/home/claude/sims` (see Testing below).
...
## Sandbox Loop v0.3.1 — what it does
...
`/home/claude/sims` holds Node harnesses that `eval` the mod's real `main.js` in strict mode
...
Run them with `node`. They are the only way to test without the game.

## Deploy workflow

Edit in `/home/claude/<mod>-mod/main.js`, bump `BUILD` in main.js and `version` in
